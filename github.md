...
- **`tools/prerender.js`** (published, no dependencies): emits one real HTML document per route — `./knowledge/index.html`, `./project/<key>/index.html`, … — reading `ROUTE_META` and the part records (name, tagline, vision) out of the built `index.html`. Canonicals, noscript links and sitemap now use clean URLs without fragments; generated blocks are idempotent; `tools/prerender-check.js` verifies unique metadata and every favicon path. Hash routes remain backward-compatible.
...
- Static regression now covers the four-layer map, route metadata, Dknowledger model and both snapshot origins; prerender validation covers all 35 `.org` and 29 `.com` clean routes. The dedicated Dknowledger repository adds its own generated source catalog and site contract tests.
- **Prepared for publication**: package 3.4.1 is integrated into the official checkouts, `.com` regenerated from the shared component, clean routes materialized, and the first public organization snapshot committed locally. Publishing and production verification are the remaining deployment steps.
